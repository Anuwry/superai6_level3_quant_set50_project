...
result by aggregating several reasoning paths **(X. Wang et al.,
2023)**. Comparing a three-call role system only with one call therefore
...
historical-data pages **(Investing.com, n.d.-a, n.d.-b)**. The
provenance manifest records source URLs, acquisition evidence, file
sizes, SHA-256 digests, row counts, date ranges, and processing method.
Public accessibility is not treated as an open-data license, provider
terms continue to apply, and raw rows are not redistributed. Date-only
