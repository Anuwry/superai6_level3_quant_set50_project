...
MODEL_NAME = "attention_lstm_cnn"
CONV_FILTERS = 32
KERNEL_SIZE = 3
CONFIG = {
    "experiment": "naive_baseline",
    "model": "Keras Attention-LSTM-CNN",
...
def build_attention_lstm_cnn_model(input_shape: tuple[int, int]):
    import tensorflow as tf
    ...
    context = tf.keras.layers.GlobalAveragePooling1D()(convolution)
...
def predict_fold(fold: FoldData, sequence_length: int = SEQUENCE_LENGTH) -> np.ndarray:
    ...
    model = build_attention_lstm_cnn_model(
    ...
    test_sequences = make_test_sequences(train_features, test_features, sequence_length)
