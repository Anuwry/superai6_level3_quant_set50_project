...
from models.pit_dern import MODEL_KEY, MODEL_LABEL
...
def _validate_summary(
    ...
    *,
    ...
    if frame.loc[:, ["model", "arm"]].duplicated().any():
...
def build_six_model_tables(
    ...
    combined["model_order"] = pd.Categorical(
        combined["model"], categories=SIX_MODEL_ORDER, ordered=True
    )
    combined["arm_order"] = pd.Categorical(
        combined["arm"], categories=ARMS, ordered=True
    )
    combined = (
    ...
    if final_arm["model"].tolist() != list(SIX_MODEL_ORDER):
        raise ValueError("Final-arm table does not preserve six-model order")
...
def build_compact_six_model_comparison(
    ...
    compact = bacc.merge(
        final_metrics, on=keys, how="inner", validate="one_to_one"
    )
...
def evaluate_promotion_gates(
    ...
    *,
    ...
    scalars = np.asarray(
        [ours_bacc, encoder_bacc, shuffled_control_bacc, parameter_delta_fraction],
        dtype=float,
    )
    if not np.isfinite(deltas).all() or not np.isfinite(scalars).all():
    ...
    gates = {
        ...
        "retrieval_mechanism_gate": float(ours_bacc) > float(encoder_bacc),
        "shuffled_control_gate": (
            float(ours_bacc) > float(shuffled_control_bacc)
        ),
        "parameter_budget_gate": float(parameter_delta_fraction) <= 0.15,
    ...
    return {
        ...
        "ours_bacc": float(ours_bacc),
        "encoder_bacc": float(encoder_bacc),
        "shuffled_control_bacc": float(shuffled_control_bacc),
        "parameter_delta_fraction": float(parameter_delta_fraction),
        **gates,
